...
#include "MovingLineSegmentCutSetUserObjectAlphaBeta.h"
#include "SubProblem.h"
#include "MooseMesh.h"
#include "VectorPostprocessorInterface.h"
#include "VectorPostprocessor.h"
#include "PointValueAtXFEMInterfaceAlphaBeta.h"

// MOOSE includes
#include "MooseError.h"

registerMooseObject("XFEMApp", MovingLineSegmentCutSetUserObjectAlphaBeta);

template <>
InputParameters
validParams<MovingLineSegmentCutSetUserObjectAlphaBeta>()
{
...
MovingLineSegmentCutSetUserObjectAlphaBeta::MovingLineSegmentCutSetUserObjectAlphaBeta(
        ...
        getParam<Real>("diffusivity_at_negative_level_set_side"))
{
  ...
  if (_cut_data.size() % line_cut_data_len != 0)
    mooseError("Length of MovingLineSegmentCutSetUserObjectAlphaBeta cut_data must be a multiple of 6.");
...
MovingLineSegmentCutSetUserObjectAlphaBeta::getCrackFrontPoints(
...
void
MovingLineSegmentCutSetUserObjectAlphaBeta::initialize()
{
  ...
  if (dynamic_cast<const PointValueAtXFEMInterfaceAlphaBeta *>(uo) == nullptr)
    mooseError(
        "UserObject casting to PointValueAtXFEMInterfaceAlphaBeta in MovingLineSegmentCutSetUserObjectAlphaBeta");

  _interface_value_uo = dynamic_cast<const PointValueAtXFEMInterfaceAlphaBeta *>(uo);
}

Real
MovingLineSegmentCutSetUserObjectAlphaBeta::calculateInterfaceVelocity(Real value_positive,
                                                              Real value_negative,
...
void
MovingLineSegmentCutSetUserObjectAlphaBeta::execute()
{
...
void
MovingLineSegmentCutSetUserObjectAlphaBeta::finalize()
{
  std::map<unsigned int, Real> value_positive = _interface_value_uo->getValueAtPositiveLevelSet();
  std::map<unsigned int, Real> value_negative = _interface_value_uo->getValueAtNegativeLevelSet();
  std::map<unsigned int, RealVectorValue> grad_positive =
