...
#include "PointValueAtXFEMInterfaceOxideMetal.h"

// MOOSE includes
#include "MooseError.h"

registerMooseObject("XFEMApp", MovingLineSegmentCutSetUserObjectOxideMetal);

template <>
InputParameters
validParams<MovingLineSegmentCutSetUserObjectOxideMetal>()
{
...
MovingLineSegmentCutSetUserObjectOxideMetal::MovingLineSegmentCutSetUserObjectOxideMetal(
        ...
        getParam<Real>("diffusivity_at_negative_level_set_side"))
{
  ...
  if (_cut_data.size() % line_cut_data_len != 0)
    mooseError("Length of MovingLineSegmentCutSetUserObjectOxideMetal cut_data must be a multiple of 6.");
...
MovingLineSegmentCutSetUserObjectOxideMetal::getCrackFrontPoints(
...
void
MovingLineSegmentCutSetUserObjectOxideMetal::initialize()
{
  ...
  if (dynamic_cast<const PointValueAtXFEMInterfaceOxideMetal *>(uo) == nullptr)
    mooseError(
        "UserObject casting to PointValueAtXFEMInterfaceOxideMetal in MovingLineSegmentCutSetUserObjectOxideMetal");

  _interface_value_uo = dynamic_cast<const PointValueAtXFEMInterfaceOxideMetal *>(uo);
}

Real
MovingLineSegmentCutSetUserObjectOxideMetal::calculateInterfaceVelocity(Real value_positive,
                                                              Real value_negative,
...
void
MovingLineSegmentCutSetUserObjectOxideMetal::execute()
{
...
void
MovingLineSegmentCutSetUserObjectOxideMetal::finalize()
{
  std::map<unsigned int, Real> value_positive = _interface_value_uo->getValueAtPositiveLevelSet();
  std::map<unsigned int, Real> value_negative = _interface_value_uo->getValueAtNegativeLevelSet();
  std::map<unsigned int, RealVectorValue> grad_positive =
